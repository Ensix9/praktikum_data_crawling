...
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="Line Sentiment Analytics",
    page_icon="https://upload.wikimedia.org/wikipedia/commons/thumb/4/41/LINE_logo.svg/960px-LINE_logo.svg.png",
    layout="wide",
...
[data-testid="stSidebar"] {
    background: #476e4f !important;
    border-right: 1px solid #c0eac9;
}

/* Tambahkan target tag "p" agar warna benar-benar teraplikasi */
[data-testid="stSidebar"] .stRadio label,
[data-testid="stSidebar"] .stRadio p {
    color: #7bd493 !important; /* Hijau tua gelap agar kontras maksimal */
    font-size: 0.88rem;
    padding: 2px 0;
...
/* Ubah warna saat kursor diarahkan (hover) menjadi warna Emerald utama Anda */
[data-testid="stSidebar"] .stRadio label:hover,
[data-testid="stSidebar"] .stRadio p:hover {
...
.page-header {
    ...
    border-bottom: 1px solid #c0eac9;
    margin-bottom: 24px;
}
...
/* ── Prediction box ── */
.pred-positive {
    background: linear-gradient(135deg, #d0efd6 0%, #e0f5e4 100%);
...
.pred-negative {
    background: linear-gradient(135deg, #fff0f0 0%, #e0f5e4 100%);
...
.stTextArea textarea {
    ...
    border: 1px solid #c0eac9 !important;
    color: #50c878 !important;
...
.stTextArea textarea:focus {
    ...
    box-shadow: 0 0 0 2px #50c87822 !important;
}
...
# ─────────────────────────────────────────────
plt.rcParams.update({
    "figure.facecolor":  "#EFFAF1",
    "axes.facecolor":    "#E0F5E4",
    "axes.edgecolor":    "#C0EAC9",
    "axes.labelcolor":   "#23482F",
    "xtick.color":       "#9EDFAE",
    "ytick.color":       "#9EDFAE",
    "text.color":        "#23482F",
    "grid.color":        "#C0EAC9",
...
@st.cache_data
def load_data():
    csv_path = os.path.join(BASE_DIR, "../data/label_Line.csv")
...
with st.sidebar:
    ...
    <div style="text-align:center; padding: 20px 0 28px 0;">
        ...
        <div style="font-weight:700; font-size:1.05rem; color:#50c878; margin-top:6px;">Line Analytics</div>
        <div style="font-size:0.72rem; color:#7bd493; margin-top:2px;">Sentiment Analysis Dashboard Aplikasi Line</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="margin-top:auto; padding: 20px 0 8px 0; border-top:1px solid #c0eac9;">
        <div style="font-size:0.72rem; color:#7bd493; text-align:center;">
            Naive Bayes · TF-IDF · 10.000 ulasan
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if "Dashboard" in menu:
    ...
    <div class="page-header">
        ...
        <p>Gambaran umum distribusi sentimen ulasan pengguna aplikasi Line</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    cards = [
        (c1, "📊", f"{total:,}", "Total Ulasan", "#50c878"),
        (c2, "😊", f"{positif:,}", "Sentimen Positif", "#50c878"),
        (c3, "😞", f"{negatif:,}", "Sentimen Negatif", "#ff4d4d"),
        (c4, "🎯", "81.05%", "Akurasi Model", "#67ce86"),
    ]
    ...
    with col_chart:
        ...
        for bar in bars:
            ax.text(
                ...
                ha='center', va='bottom', fontsize=9, color='#50c878', fontweight='600'
            )
        ...
        wedges, texts, autotexts = ax2.pie(
            ...
            colors=["#50c878", "#ff4d4d"],
            autopct="%1.1f%%",
            startangle=90,
            wedgeprops=dict(edgecolor="#effaf1", linewidth=2),
            pctdistance=0.75
        )
        for at in autotexts:
            at.set_color("#ffffff")
    ...
    with col_info:
        ...
        <div class="insight-box">
            ...
            Dataset berisi ulasan pengguna Line dari Google Play Store yang sudah dilabeli secara manual ke dalam dua kelas sentimen.
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="insight-box">
            ...
            pengguna memberikan ulasan positif, menyatakan kualitas yang bagus untuk penggunaan setiap hari.
        </div>
        <div class="insight-box">
            <strong>😞 Negatif — {neg_pct}</strong><br>
            Sebagian besar Ulasan negatif umumnya menyoroti masalah layanan yang error,masalah login dan antarmuka aplikasi.
        </div>
        """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    def make_wordcloud(text, colormap, bg="#effaf1"):
        wc = WordCloud(
            width=1000,
            height=420,
            background_color=bg,
    ...
    with tab1:
        ...
        ax.axis("off")
        fig.patch.set_facecolor("#effaf1")
        ...
        ax2.barh(tw_df["Kata"][::-1], tw_df["Frekuensi"][::-1], color="#50c878", edgecolor="none")
...
# ══════════════════════════════════════════════
    ...
    for col, label, val, color in [
        (m1, "Accuracy",  f"{accuracy*100:.2f}%",  "#50c878"),
        (m2, "Precision", f"{precision*100:.2f}%", "#67ce86"),
        (m3, "Recall",    f"{recall*100:.2f}%",    "#7bd493"),
        (m4, "F1-Score",  f"{f1*100:.2f}%",        "#8dd9a1"),
    ]:
        with col:
            st.markdown(f"""
            <div class="metric-card" style="--accent:{color}">
    ...
    with col_cm:
        ...
        sns.heatmap(
            ...
            cbar=False,
            ax=ax,
            annot_kws={"size": 14, "weight": "bold", "color": "#23482F"}
        )
...
# ══════════════════════════════════════════════
    ...
    with col_input:
        review = st.text_area(
            "Tulis ulasan Line di sini…",
            placeholder="Contoh: Driver tepat waktu, aplikasinya juga mudah digunakan!",
            height=180
        )

        examples = [
            "Service sangat ramah dan cepat sampai tujuan",
            "Aplikasi sering error",
            "Message terlalu lama untuk di load",
        ]

        st.markdown("<div style='font-size:0.8rem; color:#7bd493; margin:12px 0 6px 0;'>✨ Coba contoh ulasan:</div>", unsafe_allow_html=True)
        for ex in examples:
        ...
        if predict_btn and review.strip():
            ...
            if pred == 1:
                ...
                <div class="pred-positive">
                    ...
                    <div style="color:#7bd493; font-size:0.82rem; margin-top:6px;">
            ...
            <div style="margin-top:20px;">
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="font-size:0.82rem; color:#67ce86; font-weight:500;">Confidence Score</span>
                    <span style="font-size:0.9rem; color:#50c878; font-weight:700;">{confidence:.1f}%</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            for bar, p in zip(bars, [prob[0]*100, prob[1]*100]):
                ax.text(p + 1, bar.get_y() + bar.get_height()/2, f"{p:.1f}%", va='center', fontsize=8, color='#50c878')
            fig.tight_layout()
            st.pyplot(fig)
            ...
            st.markdown("""
            <div style="text-align:center; padding:60px 20px; color:#7bd493;">
...
# ══════════════════════════════════════════════
    ...
    <div class="page-header">
        ...
        <p>Jelajahi dan filter dataset ulasan Line</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="font-size:0.82rem; color:#7bd493; margin-bottom:12px;">
        Menampilkan <strong style="color:#50c878">{len(data):,}</strong> baris
    </div>
    """, unsafe_allow_html=True)
    ...
    st.download_button(
        "⬇️  Download CSV",
        data=csv,
        file_name="Line_sentiment_filtered.csv",
        mime="text/csv"
    )
...
# ══════════════════════════════════════════════
    ...
    with c1:
        ...
        <div class="about-card">
            ...
            <p>
                Mengklasifikasikan sentimen ulasan pengguna aplikasi Line dari Google Play Store
                ke dalam dua kelas: <strong style="color:#50c878">positif</strong> dan
    ...
    with c2:
        ...
        <div class="about-card">
            ...
            <ul>
                ...
                <li>Total: <strong style="color:#50c878">10.000 ulasan</strong></li>
    ...
    with c3:
        ...
        <div class="about-card">
            ...
            <ul>
                ...
                <li>Ekstraksi fitur: <strong style="color:#50c878">TF-IDF Vectorizer</strong></li>
                <li>Classifier: <strong style="color:#50c878">Multinomial Naive Bayes</strong></li>
    ...
    with c4:
        ...
        <div class="about-card">
            ...
            <ul>
                <li>Accuracy: <strong style="color:#50c878">91.13%</strong></li>
                <li>Precision: <strong style="color:#67ce86">95.2%</strong></li>
                <li>Recall: <strong style="color:#7bd493">92.7%</strong></li>
                <li>F1-Score: <strong style="color:#8dd9a1">93.9%</strong></li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <div style="background:#e0f5e4; border:1px solid #c0eac9; border-radius:14px; padding:20px 24px; text-align:center;">
        <span style="font-size:0.8rem; color:#67ce86;">
